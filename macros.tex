% placeholders: REMOVE/COMMENT OUT PRIOR TO SUBMISSION
%\newcommand{\todo}[1]{[\textsl{#1}]}
%\newcommand{\todo}[1]{[\textsl{\textbf{#1}}]\marginnote{TODO}}
%\newcommand{\xxx}[0]{{\color{red} XXX}\xspace}
%\newcommand{\figxxx}[0]{{\color{red} Fig.~X}\xspace}
%\newcommand{\tbw}{\emph{To be written\ldots}}

\newcommand{\parlabel}[1]{\vspace{1mm}\noindent\textbf{#1}\xspace}

\newcommand{\autosar}[0]{\textsc{Autosar}\xspace}


\newcommand{\graphscale}{0.7}

% problem names
\newcommand{\mcm}{\textsc{MCM}\xspace}
\newcommand{\bo}{\textsc{BO}\xspace}
\newcommand{\bou}{\textsc{BO$_U$}\xspace}
\newcommand{\bd}{\textsc{BD}\xspace}
\newcommand{\bdf}{\textsc{BD$_F$}\xspace}
\newcommand{\bdp}{\textsc{BD$_P$}\xspace}

% partitioning heuristics
\newcommand{\bpa}{\textsc{BPA}\xspace}

% common abbreviations
\newcommand{\eg}{{\it e.g.}\xspace}
\newcommand{\ie}{{\it i.e.}\xspace}
\newcommand{\etc}{{\it etc.}\xspace}
\newcommand{\etal}{\emph{et~al}.\xspace}
\newcommand{\apriori}{{\it a priori}\xspace}
\newcommand{\defacto}{{\it de facto}\xspace}

\newcommand{\xth}{\ensuremath{^{\text{th}}}\xspace}

\newcommand{\vs}{{vs.}\xspace}
\newcommand{\resp}{{resp.}\xspace}
%\newcommand{\resp}{{respectively}\xspace}
\newcommand{\wrt}{{w.r.t.}\xspace}

% common Math notation
\newcommand{\NAT}[0]{\mathbb{N}\xspace}
\newcommand{\fun}[1]{\mathit{#1}} % typeset as function name
\newcommand{\setsize}[1]{\left| #1 \right|}
\newcommand{\setdef}[2]{\left\{ #1 \ \left|\  #2\right.\right\}}
\newcommand{\dispsum}[0]{\displaystyle\sum}

\newcommand{\defeq}[0]{\triangleq}
%\renewcommand{\mod}{\operatorname{mod}}

% time units
\newcommand{\mus}[0]{\ensuremath{\mu s}\xspace}
\newcommand{\us}[0]{\ensuremath{\mu s}}
\newcommand{\ms}[0]{\ensuremath{\fun{ms}}\xspace}

% algorithm names
\newcommand{\kwfont}[1]{\textrm{#1}\xspace} %\small
% variable name
\newcommand{\var}[1]{\ensuremath{{\fun{#1}}}\xspace} %\small

%http://hstuart.dk/2007/08/03/programming-latex-%E2%80%94-writing-commands/
\newcommand{\mkkw}[2]{
	\newcommand{#1}[0]{\kwfont{#2}}
}

% fancy symbols and functions
\newcommand{\Alg}[0]{{\mathcal A}}
\newcommand{\Test}[0]{{\mathcal T}}
\newcommand{\Mach}[0]{{\mathcal M}}

\newcommand{\usum}[0]{u_{\mathrm{sum}}}
\newcommand{\umax}[0]{u_{\mathrm{max}}}
\newcommand{\umin}[0]{u_{\mathrm{min}}}
\newcommand{\utop}[0]{u_{\mathrm{top}}}

\newcommand{\esum}[0]{e_{\mathrm{sum}}}
\newcommand{\emax}[0]{e_{\mathrm{max}}}
\newcommand{\emin}[0]{e_{\mathrm{min}}}
\newcommand{\etop}[0]{e_{\mathrm{top}}}

\newcommand{\dsum}[0]{\delta_{\mathrm{sum}}}
\newcommand{\dmax}[0]{\delta_{\mathrm{max}}}
\newcommand{\dmin}[0]{\delta_{\mathrm{min}}}
\newcommand{\dtop}[0]{\delta_{\mathrm{top}}}

\newcommand{\prio}[0]{\mathsf Y}
\newcommand{\eprio}[0]{\mathsf y}

\newcommand{\njobs}[0]{\fun{njobs}}
\newcommand{\rsf}[0]{\fun{rsf}}

\newcommand{\numclust}[0]{K}
\newcommand{\numres}[0]{n_r}
\newcommand{\nmax}[0]{N^{\fun{max}}}
\newcommand{\pacc}[0]{p^{\fun{acc}}}

% src code
\newcommand{\src}[1]{\texttt{#1}\xspace}


% schedulers
\mkkw{\cfs}{CFS}

\mkkw{\edf}{EDF}
\mkkw{\edfwm}{EDF-WM}
\mkkw{\fp}{FP}
\mkkw{\fprm}{RM}
\mkkw{\fpdm}{DM}
\mkkw{\gedf}{G-EDF}
\mkkw{\gsnedf}{GSN-EDF}
\mkkw{\gfp}{G-FP}
\mkkw{\pedf}{P-EDF}
\mkkw{\pfp}{P-FP}
\mkkw{\cedf}{C-EDF}
\mkkw{\pssched}{PS}

\mkkw{\pfsched}{PF}
\mkkw{\pd}{PD}
\mkkw{\pds}{PD$^2$}
\mkkw{\cpds}{C-PD$^2$}

\mkkw{\jlfp}{JLFP}
\mkkw{\jldp}{JLDP}

% POSIX

\mkkw{\schedfifo}{SCHED\_FIFO}
\mkkw{\schedrr}{SCHED\_RR}
\mkkw{\schedother}{SCHED\_OTHER}
\mkkw{\schedspor}{SCHED\_SPORADIC}
\mkkw{\prioprot}{PRIO\_PROTECT}
\mkkw{\prioinh}{PRIO\_INHERIT}
\mkkw{\scheddl}{SCHED\_DEADLINE}

% locking protocols
\mkkw{\npcs}{NCP}
\mkkw{\srp}{SRP}
\mkkw{\pcp}{PCP}
\mkkw{\ppcp}{P-PCP}
\mkkw{\lbpcp}{LB-PCP}
\mkkw{\gpip}{G-PIP}
\mkkw{\msrp}{MSRP}
\mkkw{\dpcp}{DPCP}
\mkkw{\mpcp}{MPCP}
\mkkw{\mpcpvs}{MPCP-VS}
\mkkw{\fmlp}{FMLP}
\mkkw{\fmlpp}{FMLP$^{\mathrm{+}}$}
\mkkw{\rsbpp}{PRSB}
\mkkw{\npfmlpp}{NP-FMLP$^{\mathrm{+}}$}
\mkkw{\omlp}{OMLP}
\mkkw{\pip}{PIP}
\mkkw{\omip}{OMIP}
\mkkw{\rdglp}{R$^2$DGLP}
\mkkw{\okglp}{O-KGLP}
\mkkw{\kfmlp}{$k$-FMLP}
\mkkw{\prsb}{PRSB}

\mkkw{\dflp}{DFLP}
\mkkw{\mbwi}{MBWI}



% RW lock implementations
\mkkw{\pft}{PF-T}   % simple PF RW lock
\mkkw{\pfc}{PF-C}   % compact PF RW lock 
\mkkw{\pfq}{PF-Q}  % queue PF RW lock
\mkkw{\rwlin}{LX-RW}
\mkkw{\tft}{TF-T}
\mkkw{\tfq}{TF-Q}
\mkkw{\mtxt}{MX-T}
\mkkw{\mtxq}{MX-Q}


% locking protocol details
\newcommand{\FQ}[1]{\ensuremath{\mathit{FQ}_{#1}}}
\newcommand{\PQ}[1]{PQ$_{#1}$}

\newcommand{\WQ}[1]{WQ$_{#1}$}
\newcommand{\RQA}[1]{CQ$_{#1}$}
\newcommand{\RQP}[1]{DQ$_{#1}$}
\newcommand{\RQ}[2]{RQ$_{#1}^{#2}$}
\newcommand{\RQi}[1]{\RQ{#1}{1}}
\newcommand{\RQii}[1]{\RQ{#1}{2}}

\newcommand{\KQ}[1]{KQ$_{#1}$}
\newcommand{\KQq}[0]{\KQ{q}\xspace}
\newcommand{\RS}[1]{RS$_{#1}$}
\newcommand{\RSq}[0]{\RS{q}\xspace}

\newcommand{\BQ}[1]{BQ$_{#1}$}

\newcommand{\mc}[0]{\frac{m}{c}}
\newcommand{\nc}[0]{\frac{n}{c}}
\newcommand{\lmax}{\rlen^{\fun{max}}}
\newcommand{\kmin}{k^{\fun{min}}}

% references

\newcommand{\chref}[1]{Chapter~\ref{ch:#1}\xspace}
\newcommand{\chrefs}[2]{Chapters~\ref{ch:#1} and~\ref{ch:#2}\xspace}
\newcommand{\secref}[1]{\S\ref{sec:#1}\xspace}
\newcommand{\appref}[1]{Appendix~\ref{sec:#1}\xspace}
\newcommand{\secrefs}[2]{\S\ref{sec:#1} and~\S\ref{sec:#2}\xspace}
\newcommand{\apprefr}[2]{Appendices~\ref{sec:#1}--\ref{sec:#2}\xspace}
\newcommand{\figref}[1]{Fig.~\ref{fig:#1}\xspace}
\newcommand{\figrefs}[2]{Figs.~\ref{fig:#1} and~\ref{fig:#2}\xspace}
\newcommand{\figrefi}[2]{Fig.~\ref{fig:#1}(#2)\xspace}
\newcommand{\figrefr}[2]{Figs.~\ref{fig:#1}--\ref{fig:#2}\xspace}
\newcommand{\tabref}[1]{Table~\ref{tab:#1}\xspace}
\newcommand{\lemref}[1]{Lemma~\ref{lem:#1}\xspace}
\newcommand{\lemrefs}[2]{Lemmas~\ref{lem:#1} and~\ref{lem:#2}\xspace}
\newcommand{\lemrefr}[2]{Lemmas~\ref{lem:#1}--\ref{lem:#2}\xspace}
\newcommand{\consref}[1]{Constraint~\ref{lem:#1}\xspace}
\newcommand{\consrefs}[2]{Constraints~\ref{lem:#1} and~\ref{lem:#2}\xspace}
\newcommand{\consrefr}[2]{Constraints~\ref{lem:#1}--\ref{lem:#2}\xspace}

\newcommand{\thmref}[1]{Theorem~\ref{thm:#1}\xspace}
\newcommand{\defref}[1]{Def.~\ref{def:#1}\xspace}
\newcommand{\exref}[1]{Example~\ref{ex:#1}\xspace}
\newcommand{\equref}[1]{Eq.~(\ref{eq:#1})\xspace}
\newcommand{\equrefs}[2]{Eqs.~(\ref{eq:#1}) and~(\ref{eq:#2})\xspace}
\newcommand{\inequref}[1]{Inequ.~(\ref{eq:#1})\xspace}
\newcommand{\lstref}[1]{Listing~\ref{lst:#1}\xspace}
\newcommand{\pref}[1]{page~\pageref{p:#1}\xspace}
\newcommand{\algoref}[1]{Algorithm~\ref{alg:#1}\xspace}
% citations

% resource notation
\newcommand{\res}[0]{\ell}
\newcommand{\req}[0]{R}
\newcommand{\wreq}[0]{\mathcal{R}^{\fun{W}}}
\newcommand{\rreq}[0]{\mathcal{R}^{\fun{R}}}
\newcommand{\rlen}[0]{{\mathcal{L}}}
\newcommand{\bspin}[0]{s}

% complicated names
\newcommand{\litmus}{LITMUS$^{\mathrm{RT}}$\xspace}
\newcommand{\aquo}{AQuoSA\xspace}
\newcommand{\prt}{PREEMPT\_RT\xspace}
\newcommand{\livlin}{L$^4$Linux\xspace}

\newcommand{\crpd}{CRPD\xspace}

